...
const AttendanceTracker = () => {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 9) {
        ...
        if (!isNaN(id)) {
          ...
          const canSkip = Math.max(0, Math.floor((attended - targetPercentage / 100 * total) / (targetPercentage / 100)));
  ...
  const overallStats = useMemo(() => {
    ...
    const totalPresent = subjects.reduce((sum, s) => sum + s.present + s.od + s.makeup, 0);
    ...
    return { totalPresent, totalClasses, overallPercentage, belowThreshold, atRisk, safe };
  }, [subjects]);
        ...
        {overallStats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 animate-fade-in">
            <div className="glass-card rounded-2xl p-4 text-center">
              <p className="text-muted-foreground text-sm mb-1">Overall Attendance</p>
              <p className={`text-2xl font-bold ${overallStats.overallPercentage >= 75 ? 'text-success' : 'text-danger'}`}>
